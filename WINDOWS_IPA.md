# Build and install PharmaShift using only Windows
...
3. Open the repository on GitHub and select **Actions → Build unsigned PharmaShift IPA → Run workflow**.
4. When the run finishes, open it and download the **PharmaShift-unsigned-IPA** artifact.
5. Extract the downloaded ZIP and copy `PharmaShift.ipa` into this project folder if you want it stored beside the Xcode project.
...
5. Hold **Shift** while clicking the AltServer tray icon, choose **Sideload .ipa…**, and select `PharmaShift.ipa`.
